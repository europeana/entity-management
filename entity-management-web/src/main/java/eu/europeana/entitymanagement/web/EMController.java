...
public class EMController extends BaseRest {
  ...
  @Autowired
	public EMController(EntityRecordService entityRecordService,
						SolrService solrService, DereferenceServiceLocator dereferenceServiceLocator, DataSources datasources,
    ...
    @RequestMapping(value = {"/{type}/{identifier}" }, method = RequestMethod.DELETE, produces = {HttpHeaders.CONTENT_TYPE_JSONLD,
	...
	@RequestMapping(value = { "/{type}/{identifier}" }, method = RequestMethod.POST, produces = {HttpHeaders.CONTENT_TYPE_JSONLD,
			MediaType.APPLICATION_JSON_VALUE})
    ...
    @RequestMapping(value = { "/{type}/{identifier}" },method = RequestMethod.PUT, produces = {
    ...
    @GetMapping(value = { "/{type}/base/{identifier}.jsonld",
	    "/{type}/{identifier}.jsonld",  "/{type}/base/{identifier}", "/{type}/{identifier}" }, produces = {HttpHeaders.CONTENT_TYPE_JSONLD,
			MediaType.APPLICATION_JSON_VALUE})
    ...
    @RequestMapping(value = { "/{type}/base/{identifier}.xml",
	    "/{type}/{identifier}.xml", "/{type}/{identifier}", "/{type}/{identifier}" }, method = RequestMethod.GET, produces = {
		...
		public ResponseEntity<String> registerEntity(
			...
			if (!datasources.hasDataSource(creationRequestId)) {
				throw new HttpBadRequestException(String
						.format("id %s does not match a configured datasource", creationRequestId));
			}
			//in case of Organization it must be the zoho Organization
			String creationRequestType = entityCreationRequest.getType();
			if (EntityTypes.Organization.getEntityType().equals(creationRequestType) && !creationRequestId.contains(DataSources.ZOHO_ID)) {
				throw new HttpBadRequestException(String
			...
			Optional<Entity> datasourceResponseOptional = dereferenceService
					.dereferenceEntityById(creationRequestId);
