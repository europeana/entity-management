		...
		try {
			...
			for (Field field : allEntityFields) {
				...
				if (fieldType.isArray()) {
					...
					if ( fieldValuePrimaryObjectArray != null) fieldValuePrimaryObject = new ArrayList<>(Arrays.asList(fieldValuePrimaryObjectArray));
					if ( fieldValueSecondaryObjectArray != null) fieldValueSecondaryObject = new ArrayList<>(Arrays.asList(fieldValueSecondaryObjectArray));
					...
					if (fieldValuePrimaryObject==null && fieldValueSecondaryObject!=null) {
						...
						for (Object secondaryElem : fieldValueSecondaryObject) {
							if (!fieldValuePrimaryObject.contains(secondaryElem)) {
								...
								if (fieldValuePrimaryObjectChanged == false) fieldValuePrimaryObjectChanged = true;
							}
						}
					}
					
					if (fieldValuePrimaryObjectChanged) primary.setFieldValue(field, fieldValuePrimaryObject.toArray((Object[]) Array.newInstance(field.getType().getComponentType(), fieldValuePrimaryObject.size())));
				}
					...
					if (fieldValuePrimaryObjectMap != null) fieldValuePrimaryObject=new HashMap<>(fieldValuePrimaryObjectMap);
					if (fieldValueSecondaryObjectMap != null) fieldValueSecondaryObject=new HashMap<>(fieldValueSecondaryObjectMap);
					boolean fieldValuePrimaryObjectChanged = false;
					...
					if (fieldValuePrimaryObject==null && fieldValueSecondaryObject!=null) {
						...
						for (Map.Entry elemSecondary : fieldValueSecondaryObject.entrySet()) { 
				            ...
				            //keep the different preferred labels in the secondary object for the alternative label in the primary object
					...
					if (fieldValuePrimaryObjectChanged) primary.setFieldValue(field, fieldValuePrimaryObject);    
					...
					List<Object> fieldValuePrimaryObject = null;
					List<Object> fieldValueSecondaryObject = null;
					if (fieldValuePrimaryObjectList!=null) fieldValuePrimaryObject = new ArrayList<Object>(fieldValuePrimaryObjectList);
					if (fieldValueSecondaryObjectList!=null) fieldValueSecondaryObject = new ArrayList<Object>(fieldValueSecondaryObjectList);
					boolean fieldValuePrimaryObjectChanged = false;
					...
					if (fieldValuePrimaryObject==null && fieldValueSecondaryObject!=null) primary.setFieldValue(field, fieldValueSecondaryObject);					
				}
				
			}
			
			//adding the preferred labels from the secondary object to the alternative labels of primary object
			if (prefLabelsForAltLabels.size()>0) {			
				for (Field field : allEntityFields) {
		            ...
		            if(fieldName.toLowerCase().contains("alt") && fieldName.toLowerCase().contains("label")) {
		            	...
		            	if (altLabelPrimaryObjectMap!=null) altLabelPrimaryObject = new HashMap<>(altLabelPrimaryObjectMap);       	
		            	...
		            	for (Map.Entry prefLabel : prefLabelsForAltLabels.entrySet()) {
		            		...
		            		List<Object> altLabelPrimaryObjectList = (List<Object>) altLabelPrimaryObject.get(altLabelFieldNamePrefix+altLabelCharacterSeparator+keyPrefLabelEnding);
		            		List<Object> altLabelPrimaryValue = null;
		            		if (altLabelPrimaryObjectList!=null) altLabelPrimaryValue = new ArrayList<>(altLabelPrimaryObjectList);
		            		else altLabelPrimaryValue=new ArrayList<>();
		            		...
		            		if (altLabelPrimaryValue.size()==0 || (altLabelPrimaryValue.size()>0 && !altLabelPrimaryValue.contains(prefLabel.getValue()))) {
			            		...
			            		if (altLabelPrimaryValueChanged == false) altLabelPrimaryValueChanged = true;
			            		altLabelPrimaryObject.put(altLabelFieldNamePrefix+altLabelCharacterSeparator+keyPrefLabelEnding, altLabelPrimaryValue);
			            	}
		            	}
    	            	if (altLabelPrimaryValueChanged) primary.setFieldValue(field, altLabelPrimaryObject);
		            	break;
		            }
				}
			}
