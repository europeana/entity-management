...
public class SolrSearchCursorIterator {
  ...
  private static final String CANNOT_PARSE_TEXT_FRAGMENT =
  ...
  /**
   * 'id' and 'type' fields are required. If query specifies fields, both must be included
   *
  ...
  private void validateQueryFields(SolrQuery solrQuery) {
    ...
    if (!Set.of(EntitySolrFields.TYPE, EntitySolrFields.ID).containsAll(fields)) {
      throw new IllegalArgumentException(
          "SolrQuery fields must either be empty or contain id and type");
    }
  }
  ...
  private SolrServiceException handleRemoteSolrException(
    ...
    return new SolrServiceException(
        "An error occurred when searching entities: "
