...
/**
 * Solr-specific @JsonFilter annotations cannot be applied to Entity classes,
 * as that would affect the JSON output returned by the API.
 *
 * Instead these mixins are used by the ObjectMapper (in SolrService) to filter out JSON properties
 * when generating the suggester payload.
 */
