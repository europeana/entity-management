...
//    
	...
	try {
		...
		if (fieldType.isAssignableFrom(String.class)) {
			...
			addConstraint(returnValue, context,
				"The entity field: " + field.getName() + " contains not allowed spaces.");
			returnValue = false;
		...
		// check the language labels for the altLabel field to be in the predefined
		// language labels
		if (fieldName.contains(altLabelFieldNamePrefix)) {
			...
			if (emLanguageCodes == null)
			    break;
			if (emLanguageCodes.getLanguages() == null)
			    break;
			for (Language language : emLanguageCodes.getLanguages()) {
			    ...
			    if (language.getAlternativeLanguages() == null)
				continue;
			    for (AlternativeLanguage alternativeLanguage : language.getAlternativeLanguages()) {
				if (alternativeLanguage.getCode() != null
					&& alternativeLanguage.getCode().contains(altLabelEnding)) {
				    if (foundAlternativeCodes == false)
					foundAlternativeCodes = true;
				    List<String> newAltLabelValue = new ArrayList<>(altLabel.getValue());
				    newAltLabel.put(altLabelFieldNamePrefix
					    + languageSeparator + language.getCode(), newAltLabelValue);
				    foundKnownCode = true;
				    break;
				}
			    }
			    if (foundKnownCode == true)
				break;
			}
			if (foundKnownCode == false) {
			    newAltLabel.put(altLabel.getKey(), altLabel.getValue());
			    addConstraint(returnValue, context, "The alternative label field: " + altLabel.getKey()
				    + " contain the language code that does not belong to the Europena languge codes.");
			    returnValue = false;

			}

		    }
		    if (foundAlternativeCodes == true)
			entity.setFieldValue(field, newAltLabel);
		}
	    }
