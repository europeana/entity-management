...
public class StepExecutionRepository extends AbstractRepository implements StepExecutionDao {
    ...
    public StepExecution getLastStepExecution(JobInstance jobInstance, String stepName) {
        ...
        Map<Long, JobExecutionEntity> jobExecutionMap = jobExecutions.stream()
                .collect(Collectors.toMap(
                        JobExecutionEntity::getJobExecutionId, jobExecutionEntity -> jobExecutionEntity));

        List<StepExecutionEntity> stepExecutions = queryGetStepExecutions(jobExecutionMap.keySet(), stepName);
    ...
    /**
     * Gets all StepExecutions for the given JobExecution Ids, with the step name.
     * Results are sorted in descending order of StartTime and StepExecutionID
     *
    ...
    private List<StepExecutionEntity> queryGetStepExecutions(Iterable<Long> jobExecutionIds, String stepName) {
        return getDataStore().find(StepExecutionEntity.class)
                .filter(
                        eq(STEP_NAME_KEY, stepName),
                        in(JOB_EXECUTION_ID_KEY, jobExecutionIds)
                ).iterator(new FindOptions()
                        .sort(
                                descending(START_TIME_KEY),
                                descending(STEP_EXECUTION_ID_KEY)
                        )).toList();
    }
