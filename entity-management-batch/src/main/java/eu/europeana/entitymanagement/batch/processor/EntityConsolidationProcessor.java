...
import eu.europeana.entitymanagement.exception.EntityValidationException;
...
import eu.europeana.entitymanagement.service.DepictionGeneratorService;
  ...
  public BatchEntityRecord doProcessing(BatchEntityRecord entityRecord)
    ...
    if (externalProxies.size() > 1) {
      ...
      for (int i = 1; i < externalProxies.size(); i++) {
        ...
        externalProxyEntity =
            entityBatchService.mergeEntities(externalProxyEntity, secondaryProxyEntity);
      }
    }
    ...
    if (isStaticDataSource) {
      ...
      consolidatedEntity =
          entityBatchService.mergeEntities(europeanaProxyEntity, externalProxyEntity);
    }

    // add external proxyIds to sameAs / exactMatch
    entityBatchService.addSameReferenceLinks(
        consolidatedEntity,
    ...
    entityBatchService.performReferentialIntegrity(consolidatedEntity);
    ...
    entityBatchService.updateConsolidatedVersion(
