...
public abstract class AbstractRepository {
    ...
    protected List<Long> queryGetJobExecutionIds(long jobInstanceId) {
        ...
        MorphiaCursor<JobExecutionEntity> cursor = getDataStore().find(JobExecutionEntity.class)
                .filter(eq(JOB_INSTANCE_ID_KEY, jobInstanceId))
                .iterator(new FindOptions()
                        .projection().include(JOB_EXECUTION_ID_KEY)
                        .limit(1));
    ...
    /**
     * Morphia doesn't allow updating fields with null values.
     * They have to be unset instead.
     * see: https://groups.google.com/g/morphia/c/pcrpS4Gmdxw
     */
