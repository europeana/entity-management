...
public class WikidataAccessService implements Dereferencer {
  ...
  public Optional<Entity> dereferenceEntityById(String wikidataUri)
    ...
    try {
      ...
      throw new WikidataAccessException(
          "Cannot parse wikidata xml response for uri: " + wikidataUri, var5);
    }
  ...
  public void saveXmlToFile(String xml, File contentFile) throws WikidataAccessException {
    try {
      ...
      if (!wasFileCreated) {
        logger.warn(
            "Content file existed, it will be overwritten: {}", contentFile.getAbsolutePath());
      }
  ...
  public void mergePropsFromWikidata(
      Organization organization, XmlOrganizationImpl wikidataOrganization)
    ...
    if (wikidataOrganizationEntity.getAltLabel() != null) {
      Map<String, List<String>> allWikidataAltLabels =
          ZohoUtils.mergeMapsWithLists(wikidataOrganizationEntity.getAltLabel(), addToAltLabelMap);
      Map<String, List<String>> mergedAltLabelMap =
          ZohoUtils.mergeMapsWithLists(allWikidataAltLabels, organization.getAltLabel());
      organization.setAltLabel(mergedAltLabelMap);
    }
    if (wikidataOrganizationEntity.getAcronym() != null) {
      Map<String, List<String>> acronyms =
          ZohoUtils.mergeMapsWithLists(
              organization.getAcronym(), wikidataOrganizationEntity.getAcronym());
    ...
    List<String> phoneList =
        ZohoUtils.mergeStringLists(organization.getPhone(), wikidataOrganizationEntity.getPhone());
    organization.setPhone(phoneList);
    List<String> mbox =
        ZohoUtils.mergeStringLists(organization.getMbox(), wikidataOrganizationEntity.getMbox());
