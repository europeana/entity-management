...
public final class ZohoUtils {
  ...
  public static List<String> stringListSupplier(Object object) {
    if (!JSONObject.NULL.equals(object)
        ...
        && CollectionUtils.isNotEmpty((List<?>) object)
        && ((List<?>) object).get(0) instanceof Choice<?>) {
      return ((List<Choice<?>>) object)
          .stream().map(Choice::getValue).map(String.class::cast).collect(Collectors.toList());
    }
   ...
   * @return
   */
  public static Map<String, List<String>> createLanguageMapOfStringList(
   ...
   */
  public static Map<String, List<String>> createLanguageMapOfStringList(
      String language, List<String> value) {
    return value == null ? null : Collections.singletonMap(toIsoLanguage(language), value);
  }

  /**
   * converts language into ISO language. Defaults it to "def" if empty
   *
   * @param language
   * @return
   */
  public static String toIsoLanguage(String language) {
    return StringUtils.isBlank(language)
        ? ZohoConstants.UNDEFINED_LANGUAGE_KEY
        : language.substring(0, 2).toLowerCase(Locale.US);
  }
